...
public class ItemHandlerTest extends EmulatorTestclass {
	...
	protected void setUp(){
		item = new Item[] {
				new Item("item1","1 kg", 0),
				new Item("new item2","2", 0),
				new Item("item 3","79 st", 1)};
	}
	...
	private void caseGetCount() {startCase("getCount of all items in a list"); try {
		...
		assertTrue(new ItemHandler(context).getCount(0) == 1);
		new ItemHandler(context).add(item[0]);
	...
	private void caseGetCountUnbought() {startCase("getCountUnbought of all items in a list"); try {
		...
		new ItemHandler(context).checked(new ItemHandler(context).getListItems(0)[0].id, true);
		assertTrue(new ItemHandler(context).getCountUnbought(0) == 1);
	trySucceeded();} catch (Exception e) {tryFailed(e);} endCase();}
	
	private void caseGetUnbought() {startCase("getUnbought items in a list"); try {
		Item[] getList = new ItemHandler(context).getUnbought(0);
		new ItemHandler(context).getUnbought(-1);
		
		assertTrue(getList.length == 1);
		assertTrue(getList[0].name.equals(item[0].name));
		assertTrue(getList[0].quantity.equals(item[0].quantity));
		assertTrue(getList[0].listId == item[0].listId);
	trySucceeded();} catch (Exception e) {tryFailed(e);} endCase();}
}